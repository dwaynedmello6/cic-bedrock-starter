...
export function summarize(rows: EvalRow[]) {
  ...
  const agg = (subset: EvalRow[]) => {
    ...
    const retrievalAccuracy =
      subset.reduce((s, r) => s + (r.retrievalCorrect ? 1 : 0), 0) / n;
    const avgRecall = subset.reduce((s, r) => s + r.retrievalRecall, 0) / n;
    const avgPrecision = subset.reduce((s, r) => s + r.retrievalPrecision, 0) / n;

    const hasRecallAtK = subset.some((r) => typeof r.recallAtK === "number");
    const avgRecallAtK = hasRecallAtK
      ? subset.reduce((s, r) => s + (r.recallAtK ?? 0), 0) / n
      : undefined;

    const hasMRR = subset.some((r) => typeof r.mrr === "number");
    const avgMRR = hasMRR
      ? subset.reduce((s, r) => s + (r.mrr ?? 0), 0) / n
      : undefined;
    ...
    return {
      ...
      avgRecallAtK: typeof avgRecallAtK === "number" ? Number(avgRecallAtK.toFixed(3)) : undefined,
      avgMRR: typeof avgMRR === "number" ? Number(avgMRR.toFixed(3)) : undefined,
    };
  };
